# === Averages limited to the Run window (start..stop) ===
...
candidates = sorted(glob.glob("/Users/chenyang/ToGoogleDrive/Gitchen/UnityMIT/SoundMapping/SoundMappingUnity/Assets/Data/default/Trajectories_1022/Setup_H_NO_20251101_170934_traj.json"), key=os.path.getmtime, reverse=True)
...
sample_hz = float(sample_hz) if isinstance(sample_hz, (int,float)) and sample_hz>0 else None
...
if t0 is not None and t1 is not None:
    mask_cte = (times >= (x0_cte if use_time else x0_cte)) & (times <= (x1_cte if use_time else x1_cte))
else:
...
# Average centroid→reference distance (NOW using run mask)
avg_err_m = float(np.mean(centroid_err[mask_cte])) if np.any(mask_cte) else float("nan")

# Survivors (unchanged)
survivors, with_g = 0, 0
for name, d in drone_tracks.items():
    g = d.get("g", None)
    if g is None or len(g) == 0: continue
    with_g += 1
    if int(g[-1]) == 1: survivors += 1

# Run total spent time (from trials; fallback to extent)
if t0 is not None and t1 is not None:
    run_total_spent_time_s = float(t1 - t0)
else:
    run_total_spent_time_s = float(total_time_s)

# Average inter-agent distances (NOW using run masks)
...
# Sidecar metrics (keeps old fields + uses run-restricted values)
...
# -------- Plot helpers (same visuals, shaded Run) --------
...
# draw each drone path + mark its endpoint
for name, d in drone_tracks.items():
    plt.plot(d["x"], d["z"], alpha=0.25)
    if len(d["x"]) > 0:
        last_x, last_z = d["x"][-1], d["z"][-1]
        last_g = None
        if d.get("g") is not None and len(d["g"]) == len(d["x"]):
            last_g = int(d["g"][-1])
        # survivors (g==1) use a filled circle; others use an 'x'
        if last_g == 1:
            plt.scatter(last_x, last_z, s=30, marker="o")
        else:
            plt.scatter(last_x, last_z, s=50, marker="x")
...
plt.xlabel("Time (s)" if use_time else ("Frame index" + (f" (~{sample_hz:.1f} Hz)" if sample_hz else "")))
